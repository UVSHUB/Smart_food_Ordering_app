...
const AddFoodScreen = ({ navigation }) => {
  ...
  const handleAddSubmit = async () => {
    ...
    try {
      await axios.post(
        BASE_URL,
        { name, price: parseFloat(price), category, image, description },
      ...
      console.log('Error adding food:', err.response?.data || err.message);
      Alert.alert('Error', err.response?.data?.message || 'Failed to add food item.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <View style={s.safeArea}>
      ...
      >
        ...
        <View style={s.headerCard}>
          <MaterialIcons name="restaurant-menu" size={36} color={C.walnut} style={{ marginRight: 14 }} />
          <View>
            <Text style={s.headerTitle}>New Menu Item</Text>
        ...
        >
          {loading ? (
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <MaterialIcons name="add-circle-outline" size={20} color={C.cream} style={{ paddingTop: 2, marginRight: 8 }} />
              <Text style={s.submitBtnText}>Add to Menu</Text>
            </View>
          )}
        </TouchableOpacity>
