...
const EditReviewScreen = ({ route, navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!userToken) {
      Alert.alert('Session Expired', 'Please login again to update your review.');
      return;
    }
  ...
  return (
    <View style={s.safeArea}>
      ...
      <View style={s.topBar}>
        ...
        <Text style={s.topBarTitle}>Edit Review</Text>
        <View style={{ width: 40 }} />
      </View>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <Text style={s.submitBtnText}>Update Review</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
...
const s = StyleSheet.create({
  ...
  reviewingTxt: { fontSize: 12, color: C.textMuted, fontWeight: '600', textTransform: 'uppercase', marginBottom: 4 },
